...
if ! npm pack --pack-destination "$TEST_DIR" "$PROJECT_ROOT" > "$TEST_DIR/core-pack.log" 2>&1; then
    echo -e "${RED}❌ FAILED: could not pack @tsonic/dotnet-bindgen${NC}"
    cat "$TEST_DIR/core-pack.log"
    exit 1
fi

if ! npm pack --pack-destination "$TEST_DIR" "$PROJECT_ROOT/npm/dotnet-bindgen" > "$TEST_DIR/wrapper-pack.log" 2>&1; then
    echo -e "${RED}❌ FAILED: could not pack dotnet-bindgen wrapper${NC}"
...
{
  "name": "dotnet-bindgen-cli-pack-smoke",
  "private": true
}
EOF_JSON
...
if ! (
    ...
    echo -e "${RED}❌ FAILED: npm install of packed dotnet-bindgen packages failed${NC}"
...
if ! dotnet-bindgen generate -a "$SMOKELIB_DLL" -o "$TEST_DIR/out" > "$TEST_DIR/generate.log" 2>&1; then
    echo -e "${RED}❌ FAILED: packed dotnet-bindgen generate should succeed without --ref-dir${NC}"
    tail -50 "$TEST_DIR/generate.log"
    exit 1
fi

if [ ! -f "$TEST_DIR/out/SmokeLib.d.ts" ]; then
    echo -e "${RED}❌ FAILED: packed dotnet-bindgen did not generate declarations${NC}"
    exit 1
fi

echo "[5/6] Verifying packed CLI resolve-closure auto-discovers runtime references..."
if ! dotnet-bindgen resolve-closure -a "$SMOKELIB_DLL" > "$TEST_DIR/closure.json" 2> "$TEST_DIR/closure.err"; then
    echo -e "${RED}❌ FAILED: packed dotnet-bindgen resolve-closure should succeed without --ref-dir${NC}"
...
if ! grep -Fq '"name":"System.Private.CoreLib"' "$TEST_DIR/closure.json"; then
    echo -e "${RED}❌ FAILED: packed dotnet-bindgen closure missing System.Private.CoreLib${NC}"
    exit 1
fi

echo "[6/6] Verifying wrapper resolves to packed core..."
if [ ! -x "$TEST_DIR/tooling/node_modules/.bin/dotnet-bindgen" ]; then
