...
F_SIGNATURES=$(jq -r '.targetSurface.types[] | select((.targetName // "") | contains("OverloadRefInOut")) | .methods[] | select(.targetName=="F") | .normalizedSignature' "$BINDINGS")
...
H_MOD=$(jq -r '.targetSurface.types[] | select((.targetName // "") | contains("OverloadRefInOut")) | .methods[] | select(.targetName=="H") | .parameterModifiers[0].modifier // "MISSING"' "$BINDINGS")
if [ "$H_MOD" != "ref" ]; then
...
K_MOD=$(jq -r '.targetSurface.types[] | select((.targetName // "") | contains("OverloadRefInOut")) | .methods[] | select(.targetName=="K") | .parameterModifiers[0].modifier // "MISSING"' "$BINDINGS")
if [ "$K_MOD" != "out" ]; then
...
L_MOD=$(jq -r '.targetSurface.types[] | select((.targetName // "") | contains("OverloadRefInOut")) | .methods[] | select(.targetName=="L") | .parameterModifiers[0].modifier // "MISSING"' "$BINDINGS")
if [ "$L_MOD" != "in" ]; then
...
BYREF_METHODS=$(jq -r '.targetSurface.types[] | select((.targetName // "") | contains("OverloadRefInOut")) | .methods[] | select((.stableId // "") | contains("&")) | .targetName' "$BINDINGS" | sort -u)

MISSING_MODIFIERS=0
for method in $BYREF_METHODS; do
    # Check if this method has parameterModifiers
    HAS_MODIFIERS=$(jq -r ".targetSurface.types[] | select((.targetName // \"\") | contains(\"OverloadRefInOut\")) | .methods[] | select(.targetName==\"$method\" and ((.stableId // \"\") | contains(\"&\"))) | if .parameterModifiers != null and (.parameterModifiers | length) > 0 then \"yes\" else \"no\" end" "$BINDINGS" | head -1)
...
REF_F=$(jq -r '.targetSurface.types[] | select((.targetName // "") | contains("OverloadRefInOut")) | .methods[] | select(.targetName=="F" and .parameterModifiers != null and .parameterModifiers[0].modifier == "ref") | .normalizedSignature' "$BINDINGS")
if [ -z "$REF_F" ]; then
...
IN_G=$(jq -r '.targetSurface.types[] | select((.targetName // "") | contains("OverloadRefInOut")) | .methods[] | select(.targetName=="G" and .parameterModifiers != null and .parameterModifiers[0].modifier == "in") | .normalizedSignature' "$BINDINGS")
if [ -z "$IN_G" ]; then
...
OUT_TRYGET=$(jq -r '.targetSurface.types[] | select((.targetName // "") | contains("OverloadRefInOut")) | .methods[] | select(.targetName=="TryGet" and .parameterModifiers != null and .parameterModifiers[0].modifier == "out") | .normalizedSignature' "$BINDINGS")
...
SIGS=$(jq -r '.targetSurface.types[] | select((.targetName // "") | contains("OverloadRefInOut")) | .methods[] | .normalizedSignature' "$BINDINGS")
