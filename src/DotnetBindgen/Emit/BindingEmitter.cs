...
public static class BindingEmitter
{
    ...
    private static string GetTypeRefName(DotnetBindgen.Model.Types.TypeReference typeRef)
    {
        return typeRef switch
        {
            DotnetBindgen.Model.Types.NamedTypeReference named => named.FullName,
            DotnetBindgen.Model.Types.NestedTypeReference nested => nested.FullReference.FullName,
            DotnetBindgen.Model.Types.GenericParameterReference gp => gp.Name,
            _ => typeRef.ToString() ?? "Unknown"
        };
    }

    private static HeritageTypeBinding GetHeritageTypeBinding(DotnetBindgen.Model.Types.TypeReference typeRef)
    {
        var named = typeRef switch
        {
            DotnetBindgen.Model.Types.NamedTypeReference n => n,
            DotnetBindgen.Model.Types.NestedTypeReference nested => nested.FullReference,
            _ => null
        };
    ...
    private static string EncodeHeritageTypeArgument(DotnetBindgen.Model.Types.TypeReference typeRef)
    {
        return typeRef switch
        {
            DotnetBindgen.Model.Types.GenericParameterReference gp => gp.Name,
            DotnetBindgen.Model.Types.NamedTypeReference named => EncodeHeritageNamed(named),
            DotnetBindgen.Model.Types.NestedTypeReference nested => EncodeHeritageNamed(nested.FullReference),
            DotnetBindgen.Model.Types.ArrayTypeReference arr => $"{EncodeHeritageTypeArgument(arr.ElementType)}[]",
            DotnetBindgen.Model.Types.PointerTypeReference ptr => $"{EncodeHeritageTypeArgument(ptr.PointeeType)}*",
            DotnetBindgen.Model.Types.ByRefTypeReference byref => EncodeHeritageTypeArgument(byref.ReferencedType),
            _ => typeRef.ToString() ?? "<opaque>"
        };
    }

    private static string EncodeHeritageNamed(DotnetBindgen.Model.Types.NamedTypeReference named)
    {
        ...
        // dotnet-bindgen deterministic encoding for generic heritage arguments:
...
///
/// TypeArguments are encoded using DotnetBindgen's deterministic "TS surface name" encoding:
///   KeyValuePair_2[[TKey,TValue]]
